...
import com.moralyzr.magickr.security.core.ports.incoming.{LoginUserByCredentials, LoginUserByCredentialsCommand, RegisterUserWithCredentials, RegisterUserWithCredentialsCommand}
...
class SecurityApi[F[_] : Async](
...
)(using marshaller: Marshallable[F]) extends LoginUserByCredentials[F]
  ...
  override def register(command: RegisterUserWithCredentialsCommand): F[Either[AuthError, User]] =
