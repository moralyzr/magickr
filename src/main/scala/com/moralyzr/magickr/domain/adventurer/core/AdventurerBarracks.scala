...
import com.moralyzr.magickr.domain.adventurer.core.commands.{
  RecruitNewAdventurerCommand,
  UpdateAdventurerInfoCommand
}
import com.moralyzr.magickr.domain.adventurer.core.errors.{
  AdventurerNotFound,
...
import com.moralyzr.magickr.domain.adventurer.core.ports.incoming.{
  RecruitAdventurer,
...
class AdventurerBarracks[F[_]: Async](
  ...
  override def update(
    ...
    updatedAdventurer <- persistAdventurer.update(adventurerToUpdate)
                           .toRight(new FailedToUpdateAdventurer())
