...
import com.moralyzr.magickr.domain.adventurer.core.errors.{AdventurerNotFound, FailedToUpdateAdventurer}
...
import com.moralyzr.magickr.domain.adventurer.core.ports.incoming.{RecruitAdventurer, UpdateAdventurerInfo}
...
class AdventurerBarracks[F[_] : Async](
  ...
  override def recruit(userId: Long, command: RecruitNewAdventurerCommand) = for {
  ...
  override def update(userId: Long, command: UpdateAdventurerInfoCommand): EitherT[F, Problem, Adventurer] = for {
    ...
    updatedAdventurer <- persistAdventurer.update(adventurerToUpdate).toRight(new FailedToUpdateAdventurer())
